...
class DWanimPushPull(QtGui.QWidget):
	...
	def createLayout(self):
		...
		if self.isMasterShot: # if scene is a master shot
			...
			##self.pushSplitAtomButton = self.addButtonWidget("Push anim Atom", mainLayout, self.doPushAnimSplitATOM)
	...
	def doMasterGetAnimATOM(self): # Import all ATOM animation from animSplit A, B and C
		...
		if self.listAssetA:
			...
			self.importAtomfile(animSplitAatomPath) # read anim ATOM file for this asset
		if self.listAssetB:
			animSplitBatomPath = self.getAtomPath(animSplit="animSplitB") 
			self.importAtomFile(animSplitBatomPath) # read anim ATOM file for this asset
		if self.listAssetC:
			animSplitCatomPath = self.getAtomPath(animSplit="animSplitC") 
			self.importAtomFile(animSplitCatomPath) # read anim ATOM file for this asset
		cmds.waitCursor(state=False)
	...
	def doPushAnimSplitGPU(self): # Export GPU caches files from the current animSplit scene.
		...
		cmds.waitCursor(state=False)
		viewMessage("All GPU's animation caches exported")
	...
	def doPushAnimSplitATOM(self): # Export ATOM animation files from the current animSplit scene.
		...
		if self.curListAsset:
			...
			print("\tlisAssets = " + str(listAssets))
			if listAssets:
				self.exportAtomFile(filterListAssets, assetsAtomPath) # export all assets in one ATOM file.
				cmds.waitCursor(state=False)
...
##		# file -force -options "precision=8;exportEdits=C:/Users/STEPH/Documents/ZOMBILLENIUM/TEST_SPLIT_ANIM/animSplitA.editMA;statics=1;baked=1;sdk=0;constraint=1;animLayers=1;selected=selectedOnly;
	...
	def exportGpuCache(self, listAssets, gpuPath): # export all selected assets as GPU individual caches in a subprocess
		print("exportGpuCache: listAssets = " + str(listAssets) + " gpuPath = " + str(gpuPath))
		cmds.select(self.listAsset, replace=True)
		...
		if len(cameras) == 1:
			...
			print("ERROR: cameras = " + str(cameras))

	# en attendant une correction de Sebastien Courtois dans le fichier "gpuCache"
	...
	def exportFromAssets(self, listAssets=None, outputDir="", shotCamera=None):
		...
		##sels = cmds.ls(selection=True) # get selection
		##print("sels = " + str(sels))
		geoList = []
