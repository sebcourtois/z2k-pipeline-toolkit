...
def Z2K_ReleaseBatch(  sceneL =[] , openInMaya=True, readOnly=True, publishMayaFile=False, releaseMayaFile =False,  backupfiles=True,
    ...
    for scene in sceneL:
        ...
        try:
            ...
            for script in prePublish_pyScriptL:
                ...
                execOutputResultD = jipeExec(scriptPath=script)
                print "execOutputResultD=", execOutputResultD
                ...
                outputD[scene]["    "+script]= tmpResult



            if not False in scrResultL:
                prePublish_result =True
                pre_text = "OK"
                prePublish_resultL.append(pre_text )
                pre_okL.append(scene)

            else:
                prePublish_result= False
                pre_text = "BAD"
                prePublish_resultL.append(pre_text)   
                pre_failL.append(scene) 
        ...
        printF( texta="*"*80 )
        printF( texta="    prePublish SCRIPTS RESULT= {0}".format(prePublish_result) )
        printF( texta="*"*80 )
        printF( texta="*"*80 )

        outputD[scene]["prePublish_SCRIPTS_RESULT"]=prePublish_result        
        ...
        try:
            ...
            if not False in scrResultL:
                preRelease_result =True
                text = "    OK:"
                resultL.append(text )
                okL.append(scene)
            else:
                preRelease_result= False
                text = "    BAD:"
                resultL.append(text)   
                failL.append(scene) 
        except Exception,err:
        ...
        printF( texta="*"*80 )
        printF( texta="    preRelease_SCRIPTS_RESULT= {0}".format(preRelease_result) )
        printF( texta="*"*80 )
        outputD[scene]["preRelease_SCRIPTS_RESULT"]=preRelease_result        

        #---------------------------------------------------------------------------------------------------------------

        # publish and release file ---------------------------------------------------------
        if prePublish_result and preRelease_result:
            # wip publishing
            if publishMayaFile in [True,1]:
                print "\n","* PUBLISHING"
                # Z2KR.publishScene(pathType=destinationAssetType , comment=theComment, sgTask=sgTask)
                printF(texta="PUBLISHING SCENE:{0}".format(scene), openMode="a")
                Z2K.publishEditedVersionSG(proj=Z2K_ReleaseToolI.proj, path_private_toPublish=path_private_toEdit, comment=theComment, 
                                            sgTask=sgTask,)
                outputD[scene]["published"]=True 
    ...
    printF(texta="@"*120 ,openMode="a")
    printF(texta="@"*120 ,openMode="a")
    printF ( texta="@ Result = {0} / {1}   -published: {3}    -released:{2}".format( len(okL),counter,len(releasedL),len(publishedL) ) )
    printF(texta="@"*120 ,openMode="a")

    for i,dico in sorted( outputD.iteritems() ):
...
sourceFolder = "Z:/06_PARTAGE/jp/ENVOI/test" #"Z:/06_PARTAGE/jp/ENVOI/150928/"
...
Z2K_ReleaseBatch(  sceneL =assetToReleaseL , 
    openInMaya=1, readOnly=0, 
    publishMayaFile=1, 
    releaseMayaFile =1,  
    backupfiles=0, deleteBackuped=0,
    prePublish_pyScriptL = [
    "C:/jipe_Local/00_JIPE_SCRIPT/PythonTree/RIG_WORKGROUP/tools/batchator_Z2K/scripts/test01A.py",
    "C:/jipe_Local/00_JIPE_SCRIPT/PythonTree/RIG_WORKGROUP/tools/batchator_Z2K/scripts/test01B.py",
    ],
    preRelease_pyScriptL=[
    "C:/jipe_Local/00_JIPE_SCRIPT/PythonTree/RIG_WORKGROUP/tools/batchator_Z2K/scripts/test02A.py",
    "C:/jipe_Local/00_JIPE_SCRIPT/PythonTree/RIG_WORKGROUP/tools/batchator_Z2K/scripts/test02B.py",
