...
def sgProject (sg, name): # find a project given the name
...
##    assetName = "set_mainStreet_default"
    ...
    for shot in shots: # for each shot
        ...
        if not os.path.exists(pathText): # metadata file exists ?
            pm.displayError("\tNo such metadata file:" + str(pathText))
            continue

        if not os.path.exists(pathCam): # camera file exists ?
            pm.displayError("\tNo such camera file:" + str(pathCam))
            continue

        print("\tOK: Data and camera readed for '{}'".format(shotName))
