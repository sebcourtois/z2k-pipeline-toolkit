...
A network folder is recommended for sharing within a studio."""
    ...
    def default(cls):
        ...
        return cls.fromName(Library.DEFAULT_NAME)
    ...
    def initLibraries(cls):
        ...
        if os.path.exists(path):
            for name in os.listdir(path):
                ...
                library_ = cls.fromName(filename)
    ...
    def sortRecords(records, sort=studiolibrary.SortOption.Ordered, order=None):
        ...
        if sort == studiolibrary.SortOption.Name:
            result = sorted(records, key=lambda r: r.name())

        elif sort == studiolibrary.SortOption.Modified:
            result = sorted(records, key=lambda r: r.mtime(), reverse=True)

        elif sort == studiolibrary.SortOption.Ordered:
