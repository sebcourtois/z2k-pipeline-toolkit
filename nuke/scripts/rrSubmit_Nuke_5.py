...
class rrJob(object):
    ...
    def writeToXMLstart(self, submitOptions ):
        jsonF = open('C:/Users/' + os.environ['USERNAME'] + '/damas_auth.json', 'r')
        davosUserNameLocal = json.load(jsonF)
        rootElement = Element("rrJob_submitFile")
        rootElement.attrib["syntax_version"] = "6.0"
        self.subE(rootElement, "DeleteXML", "1")
        self.subE(rootElement, "SubmitterParameter","DefaultClientGroup=" + '1~Comp')
        self.subE(rootElement, "SubmitterParameter","CompanyProjectName=" + '0~COMP--JOBS')
        self.subE(rootElement, "SubmitterParameter", "UserName=" + '0~{}'.format(davosUserNameLocal['224f1e9b8bda2669190aa720580ad55967eb4761']['username']))
        return rootElement
...
def getRR_Root():
    ...
    if ((sys.platform.lower() == "win32") or (sys.platform.lower() == "win64")):
        HCPath="\\\\exploitation1\\royalrendershare"
...
def rrSubmit_Nuke_Node(node, startFrame=-1, endFrame=-1):
    print ("rrSubmit v 7.0.24")
    nuke.scriptSave()
    CompName = nuke.root().name()
    if ((CompName==None) or (len(CompName)==0)):
