...
def launch(sSrcRcName, shotNames=None, dryRun=False, timestamp=None, dialogParent=None):
    ...
    if bWriteCmd:
        ...
        if cmdArgs:
            ...
            sBatFilePath = makeOutputPath(sSrcRcName, "re_submit.bat", timestamp=LAUNCH_TIME)
    ...
    submit(damShotList, sSrcRcName, dryRun=dryRun, prompt=bPrompt, sgShots=sgShots)

def submit(damShotList, sSrcRcName, dryRun=False, prompt=True, sgShots=None):
    ...
    for i, scnFile in enumerate(srcScnList):
        ...
        print scnFile, latestFile
        if not latestFile:
            validShotList[i] = None
        else:
            sVersSuffix = mkVersionSuffix(latestFile.versionFromName())

            sSuffix = "".join((sVersSuffix, '-', "4render"))
            privFile, _ = scnFile.copyToPrivateSpace(suffix=sSuffix, existing="replace",
                                                     sourceFile=latestFile)
            srcScnList[i] = privFile
    ...
    if not validShotList:
        sMsg = "None of the {} selected shots can be submitted.".format(numAllShots)
    ...
    sCode = """
import maya.cmds as mc; mc.loadPlugin('rrSubmit_Maya_Z2K.py')
